...
{
    public class TheSlushMaker : ModItem
    {
        public override void SetStaticDefaults()
        {
			Tooltip.SetDefault("Don't judge a gun by its damage.");
        }
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
			...
			Main.PlaySound(SoundID.Item48, player.position);
			type = ModContent.ProjectileType<FrozenBeansBean>();
            return true;
        }
    }
}
