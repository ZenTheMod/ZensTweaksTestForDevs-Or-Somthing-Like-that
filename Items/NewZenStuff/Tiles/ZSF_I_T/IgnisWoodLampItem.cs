...
{
    public class IgnisWoodLampItem : ModItem
    {
        public override void SetStaticDefaults()
        {
			DisplayName.SetDefault("Ignis Wood Lamp");
		}
        public override void SetDefaults()
		{
			...
			item.createTile = ModContent.TileType<IgnisWoodLampPlaced>();
			item.width = 10;
			item.height = 24;
			item.value = 500;
		}
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(ModContent.ItemType<Tree.IgnisWood>(), 3);
    ...
    public class IgnisWoodLampPlaced : ModTile
    {
		public override void SetDefaults()
		{
			...
			AddMapEntry(new Color(250, 0, 0), Language.GetText("MapObject.FloorLamp"));
		}

		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			Item.NewItem(i * 16, j * 16, 16, 48, ModContent.ItemType<IgnisWoodLampItem>());
		}
			...
			if (!Main.gamePaused && Main.instance.IsActive && (!Lighting.UpdateEveryFrame || Main.rand.NextBool(4)))
			{
				...
				if (Main.rand.NextBool(40) && frameX == 0)
				{
					...
					if (frameY / 18 % 3 == 0)
					{
						...
						if (dustChoice != -1)
						{
							int dust = Dust.NewDust(new Vector2(i * 16 + 4, j * 16 + 2), 4, 4, DustID.RedTorch, 0f, 0f, 100, default(Color), 1f);
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			var flameTexture = mod.GetTexture("Items/NewZenStuff/Tiles/ZSF_I_T/IgnisWoodLampFlame"); // We could also reuse Main.FlameTexture[] textures, but using our own texture is nice.
